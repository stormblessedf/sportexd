...
class _UserMeetupsScreenState extends State<UserMeetupsScreen> {
  ...
  String _filter = 'all'; // 'all', sport type name
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<MeetupModel>>(
        ...
        builder: (context, snapshot) {
          ...
          // Get unique sport types for filter chips
          ...
          final filtered = _filter == 'all'
              ? meetups
              : meetups.where((m) => m.type.name == _filter).toList();

          return Column(
            children: [
              // Summary header
              _buildSummaryHeader(meetups),

              // Filter chips
              _buildFilterChips(sportTypes, meetups),
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    return _buildMeetupCard(filtered[index]);
                  },
                ),
              ),
            ],
      ...
      child: Column(
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(Icons.groups, color: AppTheme.primary, size: 28),
